...
//        error_log('################## Paso linea 76 ooxml.class.php '.$this->archivoSalida.' ######## '.$this->nombreCompletoOdt);
...
//        error_log('################## Paso linea 106 ooxml.class.php '.$workDir);
    ...
    function abrirOdt() {
        ...
        if ($dirTmp == $this->workDir) {
            //error_log('################## Paso linea 155 ooxml.class.php '.$dirTmp);
                    ...
                    if ($creoDir) {
                        ...
                        switch ($this->SO) {
                            ...
                            default:
                                ...
                                if ($this->debug == true)
                                    echo "<hr>Verificando.. $verificacion <hr>";
                                break;
                        }
...
//        error_log('################## Paso linea 373 ooxml.class.php '.print_r($nombreVar). ' ###### '.$tempCab.' #### '.$tempCab2.' #### '.$tempCab3);
    ...
    function crear($nombreCompleto) {
        $this->stylesXml;
//        error_log('################## Paso linea 398 ooxml.class.php '.$nombreCompleto);
...
//        error_log('################## Paso linea 518 ooxml.class.php '.$archivoTmp.' #### '.$archivoFinal.' #### '.$tipoUnitario);
        $esArchivo = false;
        ...
        $this->contenido = $this->contenido;
//        error_log('###### ooxml.class.php '.$this->contenido);
        $error = '';
...
//        	( $this->debug == true) ? ( $error = "<br><b>No archivo de Texto, no guarda nada! $this->stylesXml</b>";
    ...
    function leeryReemplazarEtiquetaDEstilo() {
        ...
        if ($esDir) {
            ...
            $this->fp = fopen($this->stylesXml, 'r+');
            if ($this->fp) {
                $this->contenido = fread($this->fp, filesize(mb_convert_encoding($this->stylesXml, 'UTF-8')));
