            ...
            if ($vp == "n" and $radicar_a == "si") {
                if ($generar_numero == "no") {
                    ...
                    if (!$rs->EOF) {
                        ...
                        $anexoCombinarCorrespondencia = $assoc == 0 ? $rs->fields['anexo_combinar_correspondencia'] : $rs->fields['ANEXO_COMBINAR_CORRESPONDENCIA'];
...
//  $ext = substr(trim($linkarchivo),-3);
        ...
        if (/* $extVal!="DOCX" or $extVal!="ODT" or */ $extVal == "XML") {
            ...
            if($anexoCombinarCorrespondencia == '1'){
                //datos del remitente
            ...
            $isqlAnexo = "select anex_desc from ANEXOS where ANEX_CODIGO='$anexo' AND ANEX_RADI_NUME='$numrad'";
            $rsAnexo = $db->conn->Execute($isqlAnexo);
           
            $ra_asun =  $assoc == 0 ? $rsAnexo->fields['anex_desc'] : $rsAnexo->fields['ANEX_DESC'];
        ...
        <center>
            ...
            if ($actualizados > 0) {
                if ($ent != 1) {
                    ...
                    if($anexoCombinarCorrespondencia == 1){
                        fputs($fp, "*NOM_R*=" . caracteresEspeciales($remitente['nombre']) . "\n");
                    }else{
                        fputs($fp, "*NOM_R*=" . caracteresEspeciales($nombret_us1_u) . "\n");
                    }
                    
                    if($anexoCombinarCorrespondencia == 1){
                        fputs($fp, "*DIR_R*=" . caracteresEspeciales($remitente['dependencia']) . "\n");
                    }else{
