...
function _adodb_getmenu(&$zthis, $name, $defstr = '', $blank1stItem = true, $multiple = false, $size = 0, $selectAttr = '', $compareFields0 = true) {
    ...
    if ($blank1stItem)
        if (is_string($blank1stItem)) {
            ...
            if (sizeof($barr) == 1)
                $barr[] = '';
            $s .= "\n<option value=\"" . $barr[0] . "\">" . utf8_encode($barr[1]) . "</option>";
        } else
            $s .= "\n<option></option>";
    ...
    while (!$zthis->EOF) {
        ...
        if (sizeof($zthis->fields) > 1) {
            if (isset($zthis->fields[1]))
                $zval2 = mb_convert_encoding(rtrim($zthis->fields[1]), 'ISO-8859-2');
            else
                $zval2 = mb_convert_encoding(rtrim(next($zthis->fields)), 'ISO-8859-2');
        }
...
//			$value = " value='".htmlspecialchars($zval2)."'";
        ...
        if (is_array($defstr)) {
            ...
            if (in_array($selected, $defstr))
                $s .= "<option selected='selected'$value>" . $zval . '</option>';
            else
                $s .= "\n<option" . $value . '>' . $zval . '</option>';
        }
        else {
