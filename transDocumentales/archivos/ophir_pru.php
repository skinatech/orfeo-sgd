 ...
 */
$_ophir_odt_import_conf = array("features" => array("header" => 2, "list" => 2, "table" => 2, "footnote" => 2, "link" => 2, "image" => 2, "note" => 2, "annotation" => 2, 'table of contents' => 0), "images_folder" => "images");
...
function odt2html($odt_file, $xml_string = NULL)
{
    ...
    while ($xml->read()) {
        ...
        if ($xml->nodeType === XMLReader::END_ELEMENT) {
            ...
            //Close every opened tags. This should also handle malformed XML files
            continue;
        } elseif (in_array($xml->nodeType, array(XMLReader::ELEMENT, XMLReader::TEXT, XMLReader::SIGNIFICANT_WHITESPACE))) {
            //Handle tags
            switch ($xml->name) {
                case "#text":
                    //Text
        ...
        if ($xml->nodeType === XMLReader::ELEMENT && !$xml->isEmptyElement) {
            ...
            $elements_tree[] = array("name" => $xml->name, "tags" => $opened_tags);
        }
    }
