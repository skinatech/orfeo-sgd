...
switch ($db->driver) {
    case 'mssql':
        ...
        if ($docu1 == 3 and $buscar_docu != "")
            $sql .= " and r.RADI_CUENTAI like '%$buscar_docu%' ";
        if ($docu1 == 2 and $buscar_docu != "")
            $sql .= " and c.SGD_DIR_DOC like '%$buscar_docu%' ";
...
//	$sql.=" $c $srds $d $sbrds $ef $pross $b $r $a $x $v $edi $edifi $edi $entre $t $caja $u1 $caja2 $t1 $entre $t2 $estan $t3 $ufisica $t4 $piso $k $foli $i $fecha $j $fechafin $l $param $n $conse $p $depa $q $muni $orde";
        ...
        if (!isset($docu1))
            $docu1 = "";
        if ($docu1 == 1 and $buscar_docu != "")
            $sqld = "select NIT_DE_LA_EMPRESA from BODEGA_EMPRESAS where IDENTIFICADOR_EMPRESA like '$eesp'";

        $sqlca = "select distinct(c.RADI_NUME_RADI) AS RAD,$sqlFecha as FECH,
			r.RADI_PATH,c.SGD_DIR_DOC,r.RADI_CUENTAI,r.RADI_NUME_HOJA";

        if ($docu1 == 1 and $buscar_docu != "")
            $sqlca .= ",m.NIT_DE_LA_EMPRESA";
        $sqlca .= " from RADICADO r,SGD_DIR_DRECCIONES c";
        if ($docu1 == 1 and $buscar_docu != "")
            $sqlca .= ",BODEGA_EMPRESAS m";
        $sqlca .= " where ";
        if ($docu1 == 1 and $buscar_docu != "")
            $sqlca .= "m.NIT_DE_LA_EMPRESA LIKE '%$buscar_docu%' and r.EESP_CODI=m.IDENTIFICADOR_EMPRESA and c.RADI_NUME_RADI=r.RADI_NUME_RADI";
        elseif ($docu1 == 2 and $buscar_docu != "")
            $sqlca .= "c.SGD_DIR_DOC LIKE '%$buscar_docu%' and r.RADI_NUME_RADI=c.RADI_NUME_RADI";
        elseif ($docu1 == 3 and $buscar_docu != "")
            $sqlca .= "r.RADI_CUENTAI like '%$buscar_docu%' and r.RADI_NUME_RADI=c.RADI_NUME_RADI";
        else
            $sqlca .= "r.RADI_NUME_RADI=c.RADI_NUME_RADI";
        $sqlca .= " order by RAD";
        ...
        if (!isset($exp_edificio))
            $exp_edificio = "";
        if (!isset($buscar_ufisica))
            $buscar_ufisica = "";
        if (!isset($buscar_estan))
            $buscar_estan = "";
        if (!isset($buscar_entre))
            $buscar_entre = "";if (!isset($buscar_caja))
            $buscar_caja = "";
        if (!isset($buscar_piso))
            $buscar_piso = "";
...
//	$sql.=" $c $srds $d $sbrds $ef $pross $b $r $a $x $v $edi $edifi $edi $entre $t $caja $u1 $caja2 $t1 $entre $t2 $estan $t3 $ufisica $t4 $piso $k $foli $i $fecha $j $fechafin $l $param $n $conse $p $depa $q $muni $orde";
        ...
        $sql1 = "select SGD_EIT_NOMBRE from SGD_EIT_ITEMS where cast(SGD_EIT_COD_PADRE as char(5)) like '$exp_edificio' order by SGD_EIT_NOMBRE";
        $sql6 = "select SGD_EIT_NOMBRE from SGD_EIT_ITEMS where cast(SGD_EIT_COD_PADRE as char(5)) like '$buscar_ufisica' order by SGD_EIT_NOMBRE";
        $sql7 = "select SGD_EIT_NOMBRE from SGD_EIT_ITEMS where cast(SGD_EIT_COD_PADRE as char(5)) like '$buscar_estan' order by SGD_EIT_NOMBRE";
        $sql8 = "select SGD_EIT_NOMBRE from SGD_EIT_ITEMS where cast(SGD_EIT_COD_PADRE as char(5)) like '$buscar_entre' order by SGD_EIT_NOMBRE";
        $sql9 = "select SGD_EIT_NOMBRE from SGD_EIT_ITEMS where cast(SGD_EIT_COD_PADRE as char(5)) like '$buscar_caja' order by SGD_EIT_NOMBRE";
        $sql12 = "select SGD_EIT_NOMBRE from SGD_EIT_ITEMS where cast(SGD_EIT_COD_PADRE as char(5)) like '$buscar_piso' order by SGD_EIT_NOMBRE";

        //Modificado skina 080909
        if (!isset($edificio))
            $edificio = "";
        $sql13 = "select SGD_EIT_SIGLA from SGD_EIT_ITEMS where cast(sgd_eit_codigo as char(15)) like '$edificio'"; //-->nam0
        //$sql2="select SGD_EIT_SIGLA from SGD_EIT_ITEMS where SGD_EIT_CODIGO like '$ite1'";
        if (!isset($it1))
            $it1 = "";
        if (!isset($it2))
        ...
        if (!isset($it6))
            $it6 = "";if (!isset($bus))
            $bus = "";
        $sql2 = "select SGD_EIT_SIGLA from SGD_EIT_ITEMS where cast(sgd_eit_codigo as char(15)) like '$it1'";
        $sql3 = "select SGD_EIT_SIGLA from SGD_EIT_ITEMS where cast(sgd_eit_codigo as char(15)) like '$it2'";
        $sql4 = "select SGD_EIT_SIGLA from SGD_EIT_ITEMS where cast(sgd_eit_codigo as char(15)) like '$it3'";
        $sql5 = "select SGD_EIT_SIGLA from SGD_EIT_ITEMS where cast(sgd_eit_codigo as char(15)) like '$it4'";
        $sql10 = "select SGD_EIT_SIGLA from SGD_EIT_ITEMS where cast(sgd_eit_codigo as char(15)) like '$it5'";
        $sql11 = "select SGD_EIT_SIGLA from SGD_EIT_ITEMS where cast(sgd_eit_codigo as char(15)) like '$it6'";
        $sql14 = "select SGD_EIT_SIGLA from SGD_EIT_ITEMS where cast(sgd_eit_codigo as char(15)) like '$bus'";
