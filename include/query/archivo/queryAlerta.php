...
switch ($db->driver) {
    case 'mssql':
        ...
        $query_dep = "select $sqlConcat,depe_codi from DEPENDENCIA where depe_estado=1 ORDER BY DEPE_NOMB";
        $fecha_hoy = Date("Y-m-d");
        $sqlFechaHoy = $db->conn->DBDate($fecha_hoy);
        $sqlConcat = $db->conn->Concat("convert(char(20),s.sgd_srd_codigo,0)", "'-'", "s.sgd_srd_descrip");
        $sqlConcat2 = $db->conn->Concat("convert(char(20),su.sgd_sbrd_codigo,0)", "'-'", "su.sgd_sbrd_descrip");
        ...
        $redondeo_ag = "datepart(D, sexp.sgd_sexp_fech - " . $db->conn->sysTimeStamp . ") + (sb.sgd_sbrd_tiemag*365)";
        $redondeo_ac = "datepart(D, sexp.sgd_sexp_fech - " . $db->conn->sysTimeStamp . ") + (sb.sgd_sbrd_tiemac*365)";
