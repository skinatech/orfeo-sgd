...
/*                                    radicados que no est�n incluidos en alg�n      */
/*                                    expediente.                                    */
/* * ********************************************************************************** */
?>
<?php
...
switch ($db->driver) {
    case 'mssql':
        ...
        $queryUs = "SELECT DISTINCT ( R.RADI_NUME_RADI ) AS \"IDT_Numero Radicado\",
                 ...
                 WHERE R.RADI_DEPE_ACTU = '" . $dependenciaSalida . "'
                  AND R.RADI_NUME_RADI NOT IN
                  ( SELECT DISTINCT( EXP.RADI_NUME_RADI )  FROM SGD_EXP_EXPEDIENTE EXP )
                  AND R.RADI_NUME_RADI NOT IN ( 
                  SELECT DISTINCT( R.RADI_NUME_RADI )  FROM ANEXOS A, RADICADO R
                    WHERE R.RADI_NUME_RADI = A.RADI_NUME_SALIDA AND R.RADI_NUME_RADI <> A.ANEX_RADI_NUME)
    ...
    case 'mysql':
        ...
        // Tipo de Radicado
        if ($tradSel != 0) {
            $queryUs .= " AND " . $db->conn->substr . "( R.RADI_NUME_RADI, 15, 15) ='" . $tradSel . "'";
        }
        $queryUs .= " ORDER BY " . $order . " " . $orderTipo;
        // print $queryUs;
        break;
