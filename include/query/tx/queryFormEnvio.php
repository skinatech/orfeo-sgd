...
switch ($db->driver) {
    case 'mssql': {
            switch ($tmp_arr_id) {
				...
				from
				    radicado b
		 		where
		 		    b.RADI_NUME_RADI is not null ' . $whereFiltro . '
                                    ' . $whereCarpeta . '
                                    order by ' . $order . ' ' . $orderTipo;
                    }break;
                case 1: {
                        $radi_nume_radi = "i.RADI_NUME_RADI)";
                        $tmp_cad1 = $radi_nume_radi;
                        $tmp_cad2 = $radi_nume_radi;
                        $isql = 'select b.RADI_NUME_RADI as "IMG_Numero Radicado"
					,b.RADI_PATH as "HID_RADI_PATH"
					,b.RADI_FECH_RADI as "HOR_RAD_FECH_RADI"
					,' . $sqlFecha . ' as "DAT_Fecha Radicado"
            ...
            switch ($tmp_arr_id) {
                case 0: {
                        $isql = 'select b.RADI_NUME_RADI    as "IMG_Numero Radicado"'
                                ...
                                from 
                                    radicado b
                                where b.RADI_NUME_RADI is not null ' . $whereFiltro . ' ' . $whereCarpeta . ' order by ' . $order . ' ' . $orderTipo;
                    }break;
                case 1: {
                        $radi_nume_radi = "i.RADI_NUME_RADI";
    ...
    default: {
            switch ($tmp_arr_id) {
                ...
                case 2: {
                        $radi_nume_radi = "b.RADI_NUME_RADI";
                        $tmp_cad1 = $db->conn->concat("'0'", "'-'", $radi_nume_radi);
                        $tmp_cad2 = $db->conn->concat('i.info_codi', "'-'", $radi_nume_radi);
                        $isql = 'select
                                    ...
                                    ,' . $radi_nume_radi . '        as "HID_Numero Radicado"
                                    ,i.info_codi                    as "HID_InfoCodi"
                                    ,substring(i.info_desc,1,300)   as "Descripcion"
                                    ,u.USUA_NOMB                    as "Informado Por"
                                    ,' . $tmp_cad2 . '              as  "CHK_CHKANULAR"
                            from
                                    informados i, radicado b,  usuario u
