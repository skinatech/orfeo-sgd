    ...
    function insertar_expediente($num_expediente, $radicado, $depe_codi, $usua_codi, $usua_doc) {
        ...
        if (!$rs->EOF or $creadoOld == 1) {
            ...
            if (strpos($radicado, ',')) {
                ...
                /* Se consulta si ya existe la relacion entre expediente y radicado, cuando el estado es '2' = excluido. */
                $query2 = "SELECT * FROM SGD_EXP_EXPEDIENTE e WHERE e.SGD_EXP_NUMERO='$num_expediente' AND e.RADI_NUME_RADI='$radicado' AND e.SGD_EXP_ESTADO='2'";
                $rs_expediente = $this->db->conn->query($query2); 

                if($rs_expediente or count($rs_expediente->fields['SGD_EXP_NUMERO']) == 0) {
                    //Si no trae coma solo viene uno
                    $queryCreate = "insert into SGD_EXP_EXPEDIENTE(SGD_EXP_NUMERO, RADI_NUME_RADI,SGD_EXP_FECH,DEPE_CODI,USUA_CODI,USUA_DOC,SGD_EXP_ESTADO )
                    VALUES ('$num_expediente', '$radicado' ," . $sqlFechaHoy . ",'$depe_codi' ,'$usua_codi' ,'$usua_doc','$estado_expediente')";
                    $rs_create = $this->db->conn->query($queryCreate);
                    
                } else {

                    //Si ya existe y desea volver a incluir un expediente excluido actualiza los siguientes datos.
                    $query = "UPDATE SGD_EXP_EXPEDIENTE SET SGD_EXP_FECH = ".$sqlFechaHoy." , DEPE_CODI='$depe_codi', USUA_CODI='$usua_codi', USUA_DOC='$usua_doc', SGD_EXP_ESTADO='$estado_expediente' WHERE SGD_EXP_NUMERO='$num_expediente' AND RADI_NUME_RADI='$radicado'";
                    $rs_update = $this->db->conn->query($query);

                    if (!$rs_update) {  //Si actualizo BD correctamente
                        echo "Fallo la inclusion del expediente";
                    }
                }
            ...
            if (!$rs) {
                ...
                $q_excluido = "SELECT SGD_EXP_ESTADO FROM SGD_EXP_EXPEDIENTE WHERE SGD_EXP_NUMERO = '" . $num_expediente . "' AND RADI_NUME_RADI in ('$tmp[$i]' )";
                $rs_excluido = $this->db->conn->query($q_excluido);

                if ($rs_excluido->fields["sgd_exp_estado"] == 2) {
                    $q_update = "UPDATE SGD_EXP_EXPEDIENTE  SET SGD_EXP_ESTADO = 0 WHERE SGD_EXP_NUMERO = '" . $num_expediente . "'";
