...
$sqlF = " select firma_mecanica, descarga_arc_original from usuario where usua_login ='".$_SESSION['krd']."' ";
$rsF = $db->conn->Execute($sqlF);

if( $rsF->fields["FIRMA_MECANICA"] == 1) {
    ...
    $filename_mecanica = $dir_raiz.'/bodega/firmas_usuarios/firma_usuario_'.$usua_doc.'.png';
    ...
    if(is_file( $filename_mecanica ) ){
        ...
        */
        ...
        if(is_file($filename_ori)){
            ...
            $document->save($filename_ori.'png');


            /*** Se convierte con la libreria UNOCON un archivo en PDF ***/
            exec('unoconv -f pdf -o "'.$filename_pdf.'" "'.$filename_ori.'png" ');
            /*** Se elimina el archivo finalizado en QR ya que es temporal ***/
            exec(' rm '.$filename_ori.'png ' );

        }else{
            ?>
            <font size='3' color='#000000'>
                <a class='vinculos' href="../verradicado.php?<?= session_name().'='.session_id().'&krd='.$krd ?>&verrad=<?= $num_radi_salida ?>&menu_ver_tmp=2&verrad=<?= $radicar_documento ?>">Volver al radicado <?= $num_radi_salida ?></a></b></span><br>
                <?php
        ...
        <font size='3' color='#000000'>
            <a class='vinculos' href="../verradicado.php?<?= session_name().'='.session_id().'&krd='.$krd ?>&verrad=<?= $num_radi_salida ?>&menu_ver_tmp=2&verrad=<?= $radicar_documento ?>">Volver al radicado <?= $num_radi_salida ?></a></b></span><br>
            <?php
            die("<span class='etextomenu'><b>El usuario no tiene configurada la imagen de la firma </span><BR>");
            ?>
        </font>
        <?php
    }
    ...
    if(is_file( $filename_pdf )){
        ...
        $sqlR = "UPDATE anexos SET doc_firmado_mecanica = '1' WHERE radi_nume_salida = '".$num_radi_salida."' ";
