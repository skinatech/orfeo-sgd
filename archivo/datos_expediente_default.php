...
$sqlrad = "select RADI_NUME_RADI FROM SGD_EXP_EXPEDIENTE WHERE SGD_EXP_NUMERO LIKE '$num_expediente' AND radi_nume_radi = '$nurad' order by RADI_NUME_RADI";
...
//echo '</br>++++++++ $exp_piso2 '.$exp_piso2.' ++++++ '.$sqm .' ++++ </br> ';
if ($exp_item12 != "") {
    //modificado skina conversion de variables 
    $ttp = "select sgd_eit_nombre from sgd_eit_items where cast(sgd_eit_codigo as char(5)) like '$exp_item12'";
    $rs = $db->conn->Execute($ttp);

//    echo '</br>++++++++ $exp_item12 '.$exp_item12.' ++++++ '.$ttp .' ++++ </br> ';
    $tmp = $assoc == 0 ? $rs->fields['sgd_eit_nombre'] : $rs->fields['SGD_EIT_NOMBRE'];
    $tmp1 = explode(' ', $tmp);
    if ($tmp1[0] == "CAJA")
        $exp_caja = $exp_item12;
...
//    echo '</br>++++++++++++ $exp_item22 '.$exp_item22.' ++++++ '.$ttp .' ++++ </br> ';
    $tmp = $assoc == 0 ? $rs->fields['sgd_eit_nombre'] : $rs->fields['SGD_EIT_NOMBRE'];
    $tmp1 = explode(' ', $tmp);
    if ($tmp1[0] == "CAJA")
        $exp_caja = $exp_item22;
...
//    echo '</br>+++++++++++++++ $exp_item31 '.$exp_item31.' ++++++ '.$ttp .' ++++ </br> ';
    $tmp = $assoc == 0 ? $rs->fields['sgd_eit_nombre'] : $rs->fields['SGD_EIT_NOMBRE'];
    $tmp1 = explode(' ', $tmp);
    if ($tmp1[0] == "CAJA")
        $exp_caja = $exp_item31;
...
//    echo '</br>+++++++++++++++++ $exp_entre '.$exp_entre.' ++++++ '.$ttp .' ++++ </br> ';
    $tmp = $assoc == 0 ? $rs->fields['sgd_eit_nombre'] : $rs->fields['SGD_EIT_NOMBRE'];
    $tmp1 = explode(' ', $tmp);
    if ($tmp1[0] == "CAJA")
        $exp_caja = $exp_entre;
...
//    echo '</br>+++++++++++++++++++++++ $exp_caja2 '.$exp_caja2.' ++++++ '.$ttp .' ++++ </br> ';
    $tmp = $assoc == 0 ? $rs->fields['sgd_eit_nombre'] : $rs->fields['SGD_EIT_NOMBRE'];
    $tmp1 = explode(' ', $tmp);
    if ($tmp1[0] == "CAJA")
        $exp_caja = $exp_caja2;
...
while (!$rsrad->EOF) {
    $arrayRad[$j] = $assoc == 0 ? $rsrad->fields['radi_nume_radi'] : $rsrad->fields['RADI_NUME_RADI'];
    $j++;
    $rsrad->MoveNext();
}
...
//echo '+++++++++++++++++++++++ $exp_carpeta2 '.$exp_carpeta2.' ++++++ '.$sqlrad3 .' ++++ <br> ';
$cpt = $exp_carpeta;
$j = 1;
$p = 1;
$exp_folio = 0;
...
while (!$rsrad3->EOF) {
    $arrayRad2[$j] = $assoc == 0 ? $rsrad3->fields['radi_nume_radi'] : $rsrad3->fields['RADI_NUME_RADI'];
    $foli[$j] = $assoc == 0 ? $rsrad3->fields['radi_nume_hoja'] : $rsrad3->fields['RADI_NUME_HOJA'];
    $CD1[$j] = $assoc == 0 ? $rsrad3->fields['medio_m'] : $rsrad3->fields['MEDIO_M'];
    $esta[$j] = $assoc == 0 ? $rsrad3->fields['sgd_exp_estado'] : $rsrad3->fields['SGD_EXP_ESTADO'];
    ...
    if ($exp_fechaFin <= date("Y-m-d")) {
        ...
        while ($i <= $cont) {
            if ($inclu[$i] == $k) {
                ...
                $fecha_finals = date("Y-m-d", strtotime('next year'));
                //error_log('@@ linea 190 ' . $arrayRad2[1] . ',' . $num_expediente . ',' . $exp_titulo . ',' . $exp_asunto . ',' . $exp_item12 . ',' . $exp_piso2 . ',' . $exp_item31 . ',' . $exp_estante . ',' . $exp_carpeta . ',' . $exp_subexpediente . ',' . $ES . ',' . $UN . ',' . $exp_fechaIni . ',' . $fecha_finals . ',' . $exp_folio . ',' . $exp_rete . ',' . $exp_entrepa . ',' . $exp_edificio2 . ',' . $krd . ',' . $exp_item22);
